...
export default function ItemProduct({ id, name, image, price, description } : Omit<Product, 'menuId' | 'categoryId' >) {
    return (
        <div key={id} className="flex w-4/5 md:w-96 lg:w-96 mr-4 rounded  mb-4 border-2 border-slate-500 shadow-lg transform scale-100 transition-transform duration-300 hover:scale-105 hover:shadow-lg hover:cursor-pointer hover:shadow-cyan-500/50">
            <>
                <img className="w-1/2 md:w-48 lg:w-48 p-2 border-2 rounded" src={image} />
