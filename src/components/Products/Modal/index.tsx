            ...
            <div className="flex flex-col  bg-blue-200 w-1/2 h-1/2 shadow-2xl rounded-lg shadow-black  ">
                ...
                <div className="h-4/5 p-1 flex">
                    ...
                    <div className="flex flex-col justify-center items-center w-full font-medium">
                        <p className="mb-2 text-xs md:text-3xl text-center"> {selectProduct?.name}</p>
                        <p className="mb-2 text-center text-xs md:text-lg"> {selectProduct?.description}</p>
                        <p className="text-xs md:text-3xl"> {`R$ ${((selectProduct?.price)/100).toFixed(2)}`}</p>

                    </div>
                </div>
                <div className="flex justify-center items-center w-full mt-2 ">
                    <button onClick={() => {handleOrder(selectProduct)}}  className="bg-green-400 w-40 h-10 rounded-3xl hover:cursor-pointer hover:bg-green-300 mb-2"> Pedir </button>
                </div>
            </div>
        </div>
    )
